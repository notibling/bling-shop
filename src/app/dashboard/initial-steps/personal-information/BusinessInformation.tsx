...
const BusinessInformation: React.FC<IBusinessInformationProps> = ({ controller }) => {
  ...
  const countryRef = useRef<ISimpleAutocompleteRef<any>>(null);
  const residenceCountryRef = useRef<ISimpleAutocompleteRef<any>>(null);
  const commercialSectorRef = useRef<ISimpleAutocompleteRef<any>>(null);

  function handleUpdateField(key: keyof IUser, value: any) {
    setUser((prev) => ({
      ...prev,
      [key]: value,
    }));
  ...
  useEffect(() => {
    commercialSectorRef.current && commercialSectorRef.current.setValue(_.find(commercialSector, { value: user?.sex ?? '' })?.name ?? '');
    countryRef.current && countryRef.current.setValue(_.find(countries, { iso: user?.country ?? '' })?.name ?? '');
    residenceCountryRef.current && residenceCountryRef.current.setValue(_.find(countries, { iso: user?.residenceCountry ?? '' })?.name ?? '');
  }, [user]);
