...
import { TableInReviewReturns } from '@/components/Views/Dashboard/Returns';


interface InReviewReturnsProps {}

const InReviewReturns: React.FC<InReviewReturnsProps> = ({}) => {
  return (
    <>
      <Section Empty={true} className='flex-col !h-auto items-start justify-start self-start !p-0 m-0 flex gap-4 !bg-transparent rounded-md'>
        ...
        <TitleSection icon={EyeScan} iconSize={30} level='h5' title={
          <>
            <div className='w-auto h-auto flex-row justify-start items-center flex gap-1'><span>Devoluciones</span><span className='bling-status font-JetBrainsMono leading-normal  bling-status-attention dark:bling-status-attention-dark h-6 items-center justify-center '>En Revisión</span></div> 
          </>
        } description='Devoluciones con observaciones pendientes'
...
export default InReviewReturns;
