...
import { AlignVerticalSpacing, Check, Csv, DownloadMinimalistic, ListDownMinimalistic, Pause, Pdf, Pen2, Play, Printer2, TrashBin2, Xls } from '@/icons/tsx';
import { Dropdown, DropdownItem, TitleSection } from '@/components/organisms';
import { TablePausedProducts } from '@/components/Views/Dashboard/Products';


interface PausedProductsProps {}

const PausedProducts: React.FC<PausedProductsProps> = ({}) => {
  return (
    <>

      <Section Empty={true} className='flex-col  items-start justify-start h-fit !p-0 m-0 flex gap-4 !bg-transparent rounded-md'>
      
        <TitleSection icon={Pause}
          iconSize={30}
          title={
            <>
              <div className='w-auto h-auto flex-row justify-start items-center flex gap-1'><span>Productos</span><span className='bling-status bling-status-disabled dark:bling-status-disabled-dark font-JetBrainsMono leading-normal h-6 items-center justify-center '>En Pausa</span></div> 
            </>
          } 
          description='Productos no visibles para el publico'
          level='h5'
