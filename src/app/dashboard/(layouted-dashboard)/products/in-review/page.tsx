...
import { TableInReviewProducts } from '@/components/Views/Dashboard/Products';


interface InReviewProductsProps {}

const InReviewProducts: React.FC<InReviewProductsProps> = ({}) => {
  return (
    <>
      <Section Empty={true} className='flex-col !h-auto items-start justify-start self-start !p-0 m-0 flex gap-4 !bg-transparent rounded-md'>
        ...
        <TitleSection icon={EyeScan}
          iconSize={30}
          title={
            <>
              <div className='w-auto h-auto flex-row justify-start items-center flex gap-1'><span>Productos</span><span className='bling-status font-JetBrainsMono leading-normal  bling-status-attention dark:bling-status-attention-dark h-6 items-center justify-center '>En Revisión</span></div> 
            </>
          } 
          description='Productos con observaciones pendientes'
          level='h5'
...
export default InReviewProducts;
