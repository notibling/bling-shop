...
import { Icon, Item, Title } from '@/components/atoms';
...
const ProductCategory: React.FC<ProductCategoryProps> = ({ }) => {
  ...
  const [mainCategory, setMainCategory] = useState<INestedCategories>();
  ...
  function handleSetMainCategory(id: string) {
    setMainCategory(categories.find((category) => category.id === Number(id)) ?? undefined);
  }


  useEffect(() => {
    console.log('hello', { mainCategory, categories, state: stepper.state })
    if (!mainCategory && categories.length > 0 && stepper.state?.categories?.[0]) {
      ...
      setMainCategory(parentCategory ?? undefined);
    }
  }, [categories, stepper])
