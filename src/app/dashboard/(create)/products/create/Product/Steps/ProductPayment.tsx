...
import { FaMoneyBillTransfer, FaRotate, FaShop } from 'react-icons/fa6';
...
const ProductPayment: React.FC<ProductPaymentProps> = ({ }) => {
  ...
  const togglePaymentMethod = (key: string) => {
    ...
    if (state.paymentMethods.includes(key))
      setState((prev) => ({ ...prev, paymentMethods: prev.paymentMethods.filter((item) => item !== key) }));
    else
      setState((prev) => ({ ...prev, paymentMethods: [...prev.paymentMethods, key] }));
