...
const ProductIdentity: React.FC<ProductIdentityProps> = ({ }) => {
  ...
  const removeImage = (image: IFile) => {
    ...
    stepper.setState((prev: { images: never[]; }) => {
      const _images = prev?.images ?? [];
      return { ...prev, images: _images.filter((img: { id: any; }) => img.id !== image.id) };
    });
  };
  ...
  const getBadgeClass = (count: number): string => {
    ...
    return classes[Object.keys(classes).find(key => count < parseInt(key, 10)) ?? '61'] || '';
  };

  const characterCount = stepper.state?.title?.length ?? 0;
  const badgeClass = getBadgeClass(characterCount);
