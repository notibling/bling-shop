...
interface ITableFinishedReturnsProps {}

const TableFinishedReturns: React.FC<ITableFinishedReturnsProps> = ({}) => {
  ...
  const handleDelete = async (productDisplayId: number) => {
    ...
    if (result.success) {
      toasts.success('El registro de la devolución se ha eliminado satisfactoriamente', 4000);

      setReloader((prev) => prev + 1);
    } else {
      toasts.error('Hemos tenido un problema eliminando la devolución', 4000);
    }
  };
